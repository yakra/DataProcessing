...
if (args.skipgraphs || args.errorcheck)
	...
	thr[0] = thread(MasterTmgThread, &graph_data, &graph_vector, args.graphfilepath+'/', &traveler_lists, &graphnum, &list_mtx, &all_waypoints, &et);
	...
	for (unsigned int t = 1; t < args.numthreads; t++)
		thr[t] = thread(SubgraphThread, t, &graph_data, &graph_vector, &graphnum, &list_mtx, args.graphfilepath+'/', &all_waypoints, &et);
